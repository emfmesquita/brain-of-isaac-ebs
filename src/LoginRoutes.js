...
const oauthTokenUrl = code => `https://id.twitch.tv/oauth2/token?client_id=${clientId}&client_secret=${apiSecret}&code=${code}&grant_type=authorization_code&redirect_uri=${oauthRedirectUrl}`;
...
module.exports = (app) => {
  ...
  const loginPageTemplate = handlebars.compile(fs.readFileSync(path.resolve(__dirname, "login.html")) + "");

  const sessionIdToClientPage = (req, port) => {
    const sessionId = "s:" + signature.sign(req.sessionID, process.env.SESSION_SECRET);
    const userData = req.session.userData;
    return loginPageTemplate({ ...userData, sessionId, clientUrl: `http://localhost:${port}` });
  }
